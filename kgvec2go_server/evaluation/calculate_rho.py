...
from kgvec2go_server.babelnet.babelnet_query_service import BabelNetQueryService
...
class Evaluator:
    ...
    def simlex_spearman_rho_borda(self, path_to_simlex, use_pos=False, *services):
        ...
        for entry in simlex:
            ...
            for service in services:
                ...
                if use_pos:
                    if isinstance(service, WordnetQueryService) or isinstance(
                        service, BabelNetQueryService
                    ):
                        service_similarity = service.get_similarity(
    ...
    def simlex_spearman_rho(
        self, path_to_simlex, service, nouns_only=False, use_pos=False
    ):
        simlex = self.__load_simlex(path_to_simlex)
        service_similarity = []
        simlex_similarity = []
    ...
    def men_spearman_rho(self, path_to_men, service, nouns_only=False, use_pos=False):
        ...
        for entry in men:
            if nouns_only and (entry[1] != "n" or entry[3] != "n"):
                continue
            if use_pos:
                similarity = service.get_similarity(
                    concept_1=entry[0],
                    pos_1=entry[1],
                    concept_2=entry[2],
                    pos_2=entry[3],
    ...
    def sum_scores(list_of_borda_scores):
        ...
        for score_list in list_of_borda_scores:
            for entry in score_list:
                score_card_entry = Evaluator.get_entry_of_relative_score_for_borda(
                    final_score_card, entry.w1, entry.w2
                )
                if score_card_entry is not None:
                    final_score_card.remove(score_card_entry)
                    new_entry = GsBordaEntryResult(
                        entry.w1, entry.w2, score_card_entry.score + entry.score
                    )
    ...
    def get_relative_score_for_borda(score_map):
        ...
        for entry in descending_list:
            result.append(
                GsBordaEntryResult(
                    entry.w1, entry.w2, (length - position) / normalization_score
                )
            )
            position += 1
        return result
...
def main():
    ...
    men_gs = (
        r"/work/jportisc/EmbeddingServer/gold_standards/MEN_dataset_lemma_form_full.txt"
    )
    ...
    dbpedia_service = DBpediaQueryService(
        entity_file="/work/jportisc/Walk_Generation_dbpedia_100_8_df/cache/dbpedia_entities.txt",
    ...
    print(
        "WordSim-353 (DBpedia): "
        + str(
            evaluator.wordsim_spearman_rho(wordSim_353_similarity_gs, dbpedia_service)
        )
    )
    print(
        "SimLex-999 [all] (DBpedia): "
        + str(
            evaluator.simlex_spearman_rho(
                simlex_gs, dbpedia_service, nouns_only=False, use_pos=False
            )
        )
    )
    print(
        "SimLex-999 [nouns] (DBpedia): "
        + str(
            evaluator.simlex_spearman_rho(
                simlex_gs, dbpedia_service, nouns_only=True, use_pos=False
            )
        )
    )
    print(
        "MEN [all] (DBpedia): "
        + str(
            evaluator.men_spearman_rho(
                men_gs, dbpedia_service, nouns_only=False, use_pos=False
            )
        )
    )
    print(
        ...
        + str(
            evaluator.men_spearman_rho(
                men_gs, dbpedia_service, nouns_only=True, use_pos=False
            )
        )
    )

    dbnary_service = DbnaryQueryService(
        entity_file="/work/jportisc/EmbeddingServer/dbnary/dbnary_entities.txt",
    ...
    print(
        "WordSim-353 (DBnary): "
        + str(evaluator.wordsim_spearman_rho(wordSim_353_similarity_gs, dbnary_service))
    )
    print(
        "SimLex-999 [all] (DBnary): "
        + str(
            evaluator.simlex_spearman_rho(
                simlex_gs, dbnary_service, nouns_only=False, use_pos=False
            )
        )
    )
    print(
        "SimLex-999 [nouns] (DBnary): "
        + str(
            evaluator.simlex_spearman_rho(
                simlex_gs, dbnary_service, nouns_only=True, use_pos=False
            )
        )
    )
    print(
        ...
        + str(
            evaluator.men_spearman_rho(
                men_gs, dbnary_service, nouns_only=False, use_pos=False
            )
        )
    )
    print(
        ...
        + str(
            evaluator.men_spearman_rho(
                men_gs, dbnary_service, nouns_only=True, use_pos=False
            )
        )
    )

    alod_service = AlodQueryService(
        model_file="/work/jportisc/Training_iteration_3/alod/sg200_alod_100_8_df_mc1_it3"
    )
    print(
        "WordSim-353 (ALOD): "
        + str(evaluator.wordsim_spearman_rho(wordSim_353_similarity_gs, alod_service))
    )
    print(
        "SimLex-999 [all] (ALOD): "
        + str(
            evaluator.simlex_spearman_rho(
                simlex_gs, alod_service, nouns_only=False, use_pos=False
            )
        )
    )
    print(
        "SimLex-999 [nouns] (ALOD): "
        + str(
            evaluator.simlex_spearman_rho(
                simlex_gs, alod_service, nouns_only=True, use_pos=False
            )
        )
    )
    print(
        ...
        + str(
            evaluator.men_spearman_rho(
                men_gs, alod_service, nouns_only=False, use_pos=False
            )
        )
    )
    print(
        ...
        + str(
            evaluator.men_spearman_rho(
                men_gs, alod_service, nouns_only=True, use_pos=False
            )
        )
    )

    wordnet_service = WordnetQueryService(
        entity_file="/work/jportisc/EmbeddingServer/wordnet/wordnet_entities.txt",
        model_file="/work/jportisc/Training_iteration_3/wordnet/sg200_wordnet_100_8_df_mc1_it3",
    )
    print(
        "WordSim-353 (WordNet): "
        + str(
            evaluator.wordsim_spearman_rho(wordSim_353_similarity_gs, wordnet_service)
        )
    )
    print(
        "SimLex-999 [all] (WordNet): "
        + str(
            evaluator.simlex_spearman_rho(
                simlex_gs, wordnet_service, nouns_only=False, use_pos=True
            )
        )
    )
    print(
        "SimLex-999 [nouns] (WordNet): "
        + str(
            evaluator.simlex_spearman_rho(
                simlex_gs, wordnet_service, nouns_only=True, use_pos=True
            )
        )
    )
    print(
        ...
        + str(
            evaluator.men_spearman_rho(
                men_gs, wordnet_service, nouns_only=False, use_pos=True
            )
        )
    )
    print(
        ...
        + str(
            evaluator.men_spearman_rho(
                men_gs, wordnet_service, nouns_only=True, use_pos=True
            )
        )
    )
    ...
    print(
        "WordSim-353 (Borda): "
        + str(
            evaluator.wordsim_spearman_rho_borda(
                wordSim_353_similarity_gs,
                dbpedia_service,
                dbnary_service,
                alod_service,
                wordnet_service,
    ...
    print(
        "Simlex-999 (Borda): "
        + str(
            evaluator.simlex_spearman_rho_borda(
                simlex_gs,
                True,
                dbpedia_service,
                dbnary_service,
    ...
    print(
        "MEN (Borda): "
        + str(
            evaluator.men_spearman_rho_borda(
                men_gs,
                True,
                dbpedia_service,
                dbnary_service,
